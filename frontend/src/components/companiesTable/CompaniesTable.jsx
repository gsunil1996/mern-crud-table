...
const CompaniesTable = () => {
    ...
    const [noCompaniesData, setNoCompaniesData] = useState(false);
    ...
    const getData = ({ search, gender, status, sort, page }) => {
        ...
        let config = {
            method: "get",
            url: `http://localhost:5000/companiesTable?search=${search}&gender=${gender}&status=${status}&sort=${sort}&page=${page}`,
            headers: {
        ...
        axios(config)
            .then((data) => {
                ...
                //     data.data.companiesTableData
                // );
                if (data.data.companiesTableData.length === 0) {
                    setNoCompaniesData(true);
                } else {
                    setNoCompaniesData(false);
                }
                setCompaniesData(data.data.companiesTableData)
    ...
    return (
        <div>
            ...
            <DeleteComapny
                ...
                tableRowId={tableRowId}
                getData={getData}
                page={pageCount == 0 ? 1 : pageCount == 1 ? 1 : (pageCount > 1 && CompaniesData?.length == 1) ? currentPage - 1 : currentPage}
                setCurrentPage={setCurrentPage}
            />
